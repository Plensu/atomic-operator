    ...
    def __parse_test_guids(self, _config_file):
        ...
        if test_dict:
            for key, val in test_dict.items():
                for item in self.__build_run_list(test_guids=[key], host_list=val):
            ...
            if "all" not in techniques:
                for technique in techniques:
                    if self._all_loaded_techniques.get(technique):
                        ...
                        if select_tests:
                            temp.atomic_tests = self.select_atomic_tests(self._all_loaded_techniques[technique])
                        temp.hosts = host_list
                        __run_list.append(temp)
                ...
                for key, val in self._all_loaded_techniques.items():
                    temp = self._all_loaded_techniques[key]
                    if select_tests:
                        temp.atomic_tests = self.select_atomic_tests(self._all_loaded_techniques[key])
                    temp.hosts = host_list
                    __run_list.append(temp)
            else:
                pass
